...
class RequestResolver implements ArgumentValueResolverInterface
{
	...
	public function resolve(Request $request, ArgumentMetadata $argument): iterable
	{
		...
		if ($propertyErrors) {
			throw new InvalidRequestException(($dto && method_exists($dto, 'error')) ? $dto->error() : 'Invalid request', $propertyErrors);
		}
	...
	private function mergeRequestData(Request $request, array $content): array
	{
		...
		if (count($keys = array_intersect_key($content, $routeParameters)) > 0) {
			throw new InvalidRequestException(sprintf('Parameters (%s) used as route attributes can not be used in the request body or query parameters.', implode(', ', array_keys($keys))));
		}
	...
	{
		if (1 === count($exception->getExpectedTypes())) {
